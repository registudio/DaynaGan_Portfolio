...
function builder(name: string) {
  ...
  const box = (s: Vec3, p: Vec3, mat: Mat = 'wall', r: Vec3 = [0, 0, 0]) =>
    add(new T.BoxGeometry(...s), mat, p, r);
  ...
  const finish = () => {
    for (const [m, geos] of Object.entries(buckets)) {
      const mesh = new T.Mesh(mergeGeometries(geos), palette[m as Mat]);
...
function shell(b: ReturnType<typeof builder>, depth = 7, width = 7) {
  ...
  for (let row = 0; row < 8; row++)
    for (let col = 0; col < 7; col++) {
      ...
      b.box(
        [(width - 0.5) / 7, 0.012, (depth - 0.8) / 8],
        [x, -0.515, z],
        (row + col) % 3 ? 'wall' : 'metal',
      );
      for (const side of [-1, 1])
...
export function makeInteriors(
  ...
  for (const stage of [1, 2, 3, 6, 7]) {
    ...
    if (stage === 3) {
      for (let i = 0; i < 7; i++) {
        const f = builder(`fighter-${i}`);
        f.add(new T.SphereGeometry(0.145, 12, 8), 'metal');
